...
export default function useFetch() {
    ...
    const handleError = (err) => {
      setLoading(false);
      setError(err);
      if (err && err.response && err.response.data && err.response.data.clientErrMsg) {
    ...
    const fetchData = async (url, method, payload = [], isAuth = false) => {
      const header = isAuth ? {'Content-Type': 'application/json','x-api-key': localStorage.getItem("token")} : {'Content-Type': 'application/json'};
      setLoading(true);
      setData(null);
      setError(null);
      switch(method){
        case 'get':
            await axios[method](url, { headers: header }).then(res => {
              setLoading(false);
            ...
            break;
        case 'post': 
            await axios.post(url, payload, { headers: header}).then(res => {
              setLoading(false);
              if (!res.data || !res.data.status) {
                handleCustomError(res.data);
              } else {
