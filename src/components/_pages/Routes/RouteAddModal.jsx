...
export default function RouteAddModal({show, onFetchRoutes, onCloseButtonClick}) {
  const { data: routeData, loading: routeIsLoading, fetchData: fetchAddChargeRoute } = useFetch();
    ...
    return PortalReactDOM.createPortal(
        <div className={`route-modal modal-wrapper ${show ? 'show-modal' : 'hide-modal'}`}>
            <div className="modal">
                ...
                <h3 className="route-modal-second-title">Please Enter the Details below:</h3>
                <RouteForm loadingSubmit={routeIsLoading} onSubmitForm={addRoute} eventOnCloseButtonClick={onCloseButtonClick} />
            </div>
        </div>
        , document.body
    );
