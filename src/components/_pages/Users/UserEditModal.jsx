...
export default function UserEditModal({ show, onFetchUsers, toggle, user }) {
  ...
  return PortalReactDOM.createPortal(
    <div
      className={`user-modal modal-wrapper ${show ? "show-modal" : "hide-modal"}`}
    >
      <div className="modal">
        <CloseModal onClose={toggle} />
        <h2 className="user-modal-first-title">Edit User</h2>
        <h3 className="user-modal-second-title">
          Please Edit the Details below:
        </h3>
        <UserForm
          loadingSubmit={editIsLoading}
          onSubmitForm={editUser}
          user={user}
