...
function Verification() {
  ...
  const { data: sendVerificationData, loading: sendVerificationIsLoading, fetchData: fetchSendVerification } = useFetch();
  const { data: verificationData, loading: verificationIsLoading, fetchData: fetchVerification } = useFetch();
    ...
    <div className={`register ${hasScroll ? 'has-scroll' : 'no-scroll'}`}>
      ...
      <div className="register-wrapper">
        ...
        <div className="register-content">
          ...
          <p className="register-text">Your security is our priority. We sent an email to <a className="verification-email">{location.state.email}</a> to ensure a seamless and protected sign-up process, we require email verification. Please check your inbox, then follow the instructions to finish setting up your account.</p>
          <form className="register-form">
            ...
            <button className={`register-form-button button-blue-1 ${code == '' ? 'disable' : ''}`} onClick={handleSubmit} type="submit" >
              {verificationIsLoading ? <Loader /> : 'Send'}
            </button>
            <div onClick={resendCode} className={`resend-password ${sendVerificationIsLoading ? 'disable' : ''}`}>
