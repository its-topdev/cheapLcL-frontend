...
import { PersonIcon, MailIcon, PhoneIcon, CompanyIcon, LockIcon, EyeIcon, EyeNoIcon } from '../../../constants/icons';
import "./style.scss";

function Registration() {
  const [passwordType, handleTogglePassword] = useTogglePassword();
  const { register, handleSubmit, formState: { errors }, reset } = useForm({ mode: "onChange" });
  const { data: registerData, loading: registerIsLoading, fetchData: fetchRegister } = useFetch();
  ...
  useEffect(() => {
    if (registerData != null) {
      reset();
      navigate('/verification', { state: { email: registerData.email, name: registerData.name } });
      }
  }, [registerData]);
    ...
    <div className={`register ${hasScroll ? 'has-scroll' : 'no-scroll'}`}>
      ...
      <div className="register-wrapper">
        ...
        <div className="register-content">
          ...
          <form className="register-form" onSubmit={handleSubmit(handleRegistration, handleError)}>
            ...
            <div className="register-form-input form-input-password">
              ...
              <span className="form-error">{errors && errors.password ? errors.password.message : ''}</span>
            </div>
            <button className="register-form-button button-blue-1" disabled={registerIsLoading ? true : false} type="submit" formNoValidate="formNoValidate">
