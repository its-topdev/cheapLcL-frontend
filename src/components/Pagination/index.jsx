...
export default function Pagination ({currentPage, onSetCurrentPage, pageSize, onSetPageSize, totalCount }) {
    ...
    const toPage = pageSize > totalCount ? ((currentPage -1) * pageSize) + totalCount : ((currentPage -1) * pageSize) + pageSize;

    return (
        <div className="pagination-block">
            <div>{fromPage}-{toPage > totalCount ? totalCount : toPage} of {totalCount}</div>
            <div>
                <button className="pagination-button" onClick={handlePrevPage} disabled={currentPage === 1}>
                    <ArrowPrevIcon />
                </button>
                <ul className="pagination-pages-numbers">
                    {pageNumbers}
                </ul>
